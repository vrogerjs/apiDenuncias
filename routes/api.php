...
Route::post('token', function (Request $request) {
    ...
    if (!isset($data->uid)) {
        ...
        if (!isset($code)) {
            return Redirect::to('https://web.regionancash.gob.pe/api/oauth/authorize?response_type=code&client_id=' . $client_id_oauth);
        } else {
            try {
                $client = new Client();
                $headers = [
                ...
                $request = new Psr7Request('POST', 'https://web.regionancash.gob.pe/api/auth/token', $headers, $code);
